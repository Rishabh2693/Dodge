...
if file then
	...
	sizesathiflag = false
	objective = levelchecker("modeone",score)
		------------------------------------------------physics------------
		...
		m1background = display.newRect(display.contentCenterX, display.contentCenterY, display.contentWidth, display.contentHeight )
		m1background:setFillColor( 0.9,0.9,0.99 )
		...
		local planet = display.newCircle( centerX, centerY, 150)
		local currentcolorvalue = {0.01*math.random( 0,10 ) , 0.01*math.random( 80,200 ), 0.01*math.random( 80,200 )}
		...
		function spawnEnemy(  )
			...
			local rectorcirc = math.random( 1,2 )

				if rectorcirc == 1 then
					enemy = display.newRect( enemyx, enemyy,math.random( 120,150 ), math.random( 120,150 ) )
					enemy:setFillColor(0.01*math.random( 40,90 ) , 0.01*math.random( 40,90 ), 0.01*math.random( 40,90 ))

				else

					enemy = display.newCircle( enemyx, enemyy,math.random( 60,100 ) )
					enemy:setFillColor(0.01*math.random( 40,90 ) , 0.01*math.random( 40,90 ), 0.01*math.random( 40,90 ))

				end

			enemy.name = "enemy"	
			physics.addBody( enemy, "dynamic", {bounce = 1, density = 0.1} )
			...
			function scoring( )
				...
				if shinde == true then
					...
					if math.abs(scorercirclegroup.x - planet.x) <= range and math.abs(scorercirclegroup.y - planet.y) <= range then
						...
						nextobjective = levelchecker("modeone",score)
		...
		function sizepoweractivate(  )
			...
			sizepowercard.text = gamesizeup  	
				...
				sizeflag = true
					...
					planet.alpha = 0.01
					fakeplanet = display.newCircle( tempposx, tempposy, 120 )
		...
		function solidpoweractivate(  )
			...
			if gamesolidup > 0 and shieldflag == false and sizeflag == false and solidflag == false then
				...
				solidflag = true
					...
					solidimage = display.newCircle(  planet.x, planet.y, 150 )
					solidimage:setFillColor( currentcolorvalue[1],currentcolorvalue[2]-0.2,currentcolorvalue[3]-0.2 )
...
-----------------------------------------------gameend------------------------------------------
		...
		function farewell()
			...
			local highscore = highscore_checker(score,0,0,coinsnumber + (nextobjective - objective)/10 + (nextobjective - objective)*nextobjective/10000,gamesizeup,gameshieldup,gamesolidup)

...
if file then
	...
	sizesathiflag = false
	objective = levelchecker("modethree",score)
		------------------------------------------------physics------------
		...
		m3background = display.newImage( "Space_Backdrop.jpg",display.contentCenterX,display.contentCenterY )
		m3background.width = 2*centerX
		m3background.height = 2*centerY
		...
		local planet = display.newImage( "Planet.png",centerX,centerY )
		...
		physics.addBody( planet, "static",physicsData:get("Planet"))
		...
		function spawnEnemy(  )
			...
			local enemylist = {"UFO","Object","Satellite"}

			local rectorcirc = math.random( 1,3 )

				if rectorcirc == 3 then
					enemy = display.newImage("Satellite.png")
					--enemy:setFillColor(0.01*math.random( 40,90 ) , 0.01*math.random( 40,90 ), 0.01*math.random( 40,90 ))

				elseif rectorcirc == 1 then
					enemy = display.newImage("UFO.png")
					--enemy:setFillColor(0.01*math.random( 40,90 ) , 0.01*math.random( 40,90 ), 0.01*math.random( 40,90 ))

				elseif rectorcirc == 2 then

					enemy = display.newImage("Object.png")
					--enemy:setFillColor(0.01*math.random( 40,90 ) , 0.01*math.random( 40,90 ), 0.01*math.random( 40,90 ))
			...
			forcey = planet.y - enemy.y
				...
				if rectorcirc == 2 or rectorcirc == 3  then
...
---------------------------------------------------next objective------------------------------
		...
		objectivecard:setFillColor( 1,1,1 )
			...
			function scoring( )
				...
				if shinde == true then
					...
					if math.abs(scorercirclegroup.x - planet.x) <= range and math.abs(scorercirclegroup.y - planet.y) <= range then
						...
						nextobjective = levelchecker("modethree",score)
		...
		function sizepoweractivate(  )
			...
			sizepowercard.text = gamesizeup  	
				...
				sizeflag = true
					...
					planet.alpha = 0.01
					fakeplanet = display.newImage( "Planet_Reduced.png" )
					--fakeplanet:setFillColor( currentcolorvalue[1],currentcolorvalue[2],currentcolorvalue[3] )
					physics.addBody( fakeplanet, "static",physicsData:get("Planet_Reduced"))
		...
		sizepowercard:setFillColor( 1,1,1)
		...
		function shieldpoweractivate(  )
			...
			if gameshieldup>0 and shieldflag == false and sizeflag == false and solidflag == false then 
				...
				shieldflag = true
					...
					tempposy = planet.y
					physics.removeBody( planet )
					planetshieldouter = display.newImage( "PlanetShield.png",tempposx, tempposy, 250 )
					...
					physics.addBody( planetshieldouter, "static",physicsData:get("PlanetShield"))
		...
		shieldpowercard:setFillColor( 1,1,1 )
		...
		function solidpoweractivate(  )
			...
			if gamesolidup > 0 and shieldflag == false and sizeflag == false and solidflag == false then
				...
				solidflag = true
					...
					solidimage = display.newImage( "PlanetSolid.png" )
		...
		solidpowercard:setFillColor( 1,1,1 )
...
-----------------------------------------------gameend------------------------------------------
		...
		function farewell()
			...
			local highscore = highscore_checker(0,0,score,coinsnumber + (nextobjective - objective)/10 + (nextobjective - objective)*nextobjective/10000,gamesizeup,gameshieldup,gamesolidup)
			...
			local function farewelltransition(  )
				...
				transition.to(farewellgroup,{time=10, alpha=0.9, onComplete=temptransition})
			end
